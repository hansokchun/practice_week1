...
- 사진 EXIF 원문, SQLite 인덱스, 썸네일 캐시는 백업 제외 앱 저장소에서만 처리한다. 사용자가 게시를 확정하기 전에는 사진과 위치를 자동 전송하지 않는다.
- 게시할 때는 메타데이터를 제거한 파생 JPEG를 전송하고, 선택한 사진의 촬영일과 저장된 위치 좌표는 별도 `photos` 행 정보로 Supabase에 함께 저장한다. 위치가 없는 사진만 `lat/lng = null`이다.
...
정확한 위치는 사용자가 위치가 있는 사진을 게시할 때 선택적으로 수집되는 것으로 포함한다. Android 앱은 기기의 현재 위치 권한을 요청하지 않지만, 게시를 확정한 사진의 저장 좌표는 Supabase에 전송한다. 지도 이동·확대 및 장소 검색 요청 메타데이터는 앱 상호작용·검색 기록·진단에 포함한다.

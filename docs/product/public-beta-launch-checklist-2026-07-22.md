...
**개편 확인 항목 21개 중 19개 완료(90%), 2개 남음**
...
1. 관리자 계정을 지정하고 로그인 토큰을 갱신한 뒤 랜딩 섹션·사진 배치를 실제 저장한다.
2. iOS Safari·Android Chrome 실기기에서 새 랜딩과 상세 스크롤을 확인한다.
...
| 웹 랜딩 개편 | 미리보기·DB 마이그레이션 검증 완료 | 큰 검색창, 가로 사진 섹션, 로그인 계정 메뉴, 내 사진 안의 앨범, 거리뷰 성공·미지원 상태를 확인했다. 새 랜딩 테이블과 RLS는 원격 적용·검증했고 관리자 실제 저장과 실기기 확인 2개 남음 |

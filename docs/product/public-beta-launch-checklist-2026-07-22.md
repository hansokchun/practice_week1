...
**전체 48개 중 44개 완료(91.7%), 4개 남음 — 2026-08-30 기준**
...
- 공개 여부와 별개인 위치 정확도(`정확한 위치`·`대략 위치`) 선택과 게시 취소 동작을 구현했다. 두 정확도 모두 저장한 같은 좌표를 사용한다.
...
- 깨끗한 개발 출시 후보에서 웹 테스트 640개와 운영 빌드·성능 예산이 통과한다.
...
- 데스크톱·390px 모바일 브라우저 확인, 깨끗한 출시 후보 자동 테스트 640개, 운영 빌드
...
- 모바일 제목을 `당신만의 장소를 찾아보세요`로 바꾸고 위 여백을 늘렸으며 추천 테마 3개를 한 줄로 제한했다. 사진 가로 목록은 두 번째 사진이 중앙에서 시작해 양옆 사진이 일부 보이도록 조정했고 데스크톱 배치는 유지했다.
- 랜딩 제목 `당신만의 장소를 찾아보세요`, 추천 검색어 `제주 바다·서울 야경·일본`, 불필요한 `지금 둘러보기` 삭제를 데스크톱과 모바일에 동일하게 적용했다.
- 사진 상세는 정적 Street View 이미지를 먼저 표시하고, 마우스 올림·키보드 초점 또는 터치 화면에서 `스트리트뷰 보기`를 노출한 뒤 클릭 시에만 동적 파노라마를 만든다. 정적 이미지 실패 시에도 동적 버튼을 유지한다.
- 동적 검색으로 주변 파노라마를 찾지 못하면 조작 버튼을 숨기고 `해당 위치에 거리뷰가 없습니다`를 연한 안내 문구로 표시한다.
- Google Cloud `Travelgram` 프로젝트에서 Street View Static API를 활성화하고 HTTP 리퍼러 제한 운영 키의 허용 API에 추가했다.
...
| 자동 검증 | 통과 | 2026-08-30 `dev` 출시 후보에서 웹 `npm test` 640개, 모바일 83개 suite·309개 테스트 통과, 실패 0개 |
| 운영 빌드 | 통과 | 웹 `npm run build`·`npm run perf:budget`, 모바일 Android·iOS·Web export와 성능 예산 재검증 (2026-08-30) |
...
| 운영 배포 | 완료 | 정적→동적 거리뷰와 랜딩 문구 통합 커밋 `4cc9d3e`를 `dev`와 `main`에 반영하고 Cloudflare Pages 운영에 배포했다. 고정 배포 주소 `https://c85f0e46.practice-week1-cws.pages.dev`, 운영 주소 `https://practice-week1-cws.pages.dev`에서 정적 JPEG 200 응답, 상세의 정적 미리보기, 클릭 후 가장 가까운 동적 파노라마 전환을 확인했다. (2026-08-27) |
...
| 위치 정확도 | 구현 완료 | `정확한 위치`와 `대략 위치`는 작성자의 좌표 확신 정도이며 소유자·타 사용자·익명 지도에 같은 저장 좌표를 사용한다. 비공개 사진은 기존 RLS로 보호한다. |
...
- [x] 공개 여부와 독립적인 위치 정확도 선택, 좌표 없음, 게시 취소 정책을 정의하고 구현한다.

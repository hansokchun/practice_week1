...
- 깨끗한 운영 출시 후보에서 웹 테스트 520개와 운영 빌드가 통과한다.
- `dev`와 `main`을 `59350ba`로 맞추고 새 랜딩을 Cloudflare Pages 운영 환경에 배포했다.
...
- 데스크톱·390px 모바일 로컬 브라우저 확인, 깨끗한 출시 후보 자동 테스트 520개, 운영 빌드
...
1. 현재 오프라인인 iPhone을 연결하고 Android 기기 또는 테스트 기기를 준비해 iOS Safari·Android Chrome 실기기에서 새 랜딩과 상세 스크롤을 확인한다.
...
| 자동 검증 | 통과 | 깨끗한 `59350ba` 출시 후보에서 `npm test` 520개 통과, 실패 0개 (2026-08-27) |
| 운영 빌드 | 통과 | 깨끗한 `59350ba` 출시 후보에서 `npm run build` 재검증 (2026-08-27) |
...
| 운영 배포 | 완료 | `dev`와 `main`을 `59350ba`로 동기화하고 Cloudflare Pages 운영에 배포했다. 고정 배포 주소 `https://77752ae6.practice-week1-cws.pages.dev`, 운영 주소 `https://practice-week1-cws.pages.dev`에서 앱 셸·설정 API·자산·보안 헤더 기본 점검과 되돌리기 연습을 통과했다. (2026-08-27) |

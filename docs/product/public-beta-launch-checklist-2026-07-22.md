...
- 깨끗한 운영 출시 후보에서 웹 테스트 522개와 운영 빌드·성능 예산이 통과한다.
...
- 데스크톱·390px 모바일 로컬 브라우저 확인, 깨끗한 출시 후보 자동 테스트 522개, 운영 빌드
...
| 자동 검증 | 통과 | 깨끗한 `4cc9d3e` 출시 후보에서 `npm test` 522개 통과, 실패 0개. 공유 작업공간 전체는 별도 모바일 작업을 포함해 595개 통과 (2026-08-27) |
| 운영 빌드 | 통과 | 깨끗한 `4cc9d3e` 출시 후보에서 `npm run build`와 `npm run perf:budget` 재검증 (2026-08-27) |

...
| 자동 검증 | 통과 | 깨끗한 `e0a6799` 출시 후보에서 `npm test` 519개 통과, 실패 0개 (2026-08-27) |
| 운영 빌드 | 통과 | 깨끗한 `e0a6799` 출시 후보에서 `npm run build` 재검증 (2026-08-27) |
...
| 운영 배포 | 완료 | 주변 사진 제거 기능 커밋 `e0a6799`를 `dev`와 `main`에 반영하고 Cloudflare Pages 운영에 배포했다. 고정 배포 주소 `https://aafda5c3.practice-week1-cws.pages.dev`, 운영 주소 `https://practice-week1-cws.pages.dev`에서 앱 셸·설정 API·자산·보안 헤더 기본 점검과 되돌리기 연습을 통과했다. (2026-08-27) |

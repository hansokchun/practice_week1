...
test('photo detail renderer writes compact metadata and map handoff controls', () => {
    ...
    assert.match(source, /setPhotoImageSource\(image, photo\)/);
    ...
    assert.match(source, /setPhotoImageSource\(fullscreenImage, \{ id: state\.selectedPhotoId, url: renderedSource \}\)/);

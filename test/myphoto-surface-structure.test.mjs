...
test('home archive creation actions sit in their matching section headers', () => {
    ...
    assert.match(recentHeader, /id="btn-open-upload"[^>]*home-action-button--primary[\s\S]*add_photo_alternate[\s\S]*사진 올리기/);
    assert.match(recentHeader, /id="btn-open-photos"[^>]*home-action-button--outline[\s\S]*전체 보기[\s\S]*arrow_forward/);
    assert.match(home, /id="liked-photo-title"[^>]*class="dashboard-section-title"[\s\S]*home-title-icon--heart[\s\S]*좋아요한 사진/);
    assert.match(home, /id="btn-open-liked-photos"[^>]*home-action-button--outline[\s\S]*전체 보기[\s\S]*arrow_forward/);
    assert.match(albumHeader, /id="my-albums-title"[^>]*class="dashboard-section-title"[\s\S]*home-title-icon--album[\s\S]*여행 앨범/);
    assert.match(albumHeader, /id="btn-open-album"[^>]*home-action-button--primary[\s\S]*앨범 만들기[\s\S]*add/);
});

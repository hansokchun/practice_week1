...
test('home archive creation actions sit in their matching section headers', () => {
    ...
    assert.match(styles, /body\.is-logged-in \.home-workspace #btn-open-upload,[\s\S]*body\.is-logged-in \.home-workspace #btn-open-liked-photos\s*\{[^}]*display:\s*inline-grid;[^}]*grid-auto-flow:\s*column;[^}]*gap:\s*8px;[^}]*height:\s*42px;[^}]*box-sizing:\s*border-box;[^}]*border-radius:\s*8px;/s);
    assert.match(styles, /body\.is-logged-in \.home-workspace #btn-open-upload,[\s\S]*body\.is-logged-in \.home-workspace #btn-open-album\s*\{[^}]*grid-template-columns:\s*18px auto;[^}]*background:\s*var\(--teal-dark\);[^}]*color:\s*#ffffff;[^}]*min-width:\s*128px;[^}]*padding:\s*0 18px;/s);
...
test('home and personal photo cards omit visible caption wrappers', () => {
    ...
    assert.match(personalRenderer, /<article class="personal-photo-card \$\{isSelected \? 'is-selected' : ''\} \$\{shouldAnimateSelection \? 'is-selection-animated' : ''\}"[^>]*>\s*<button class="photo-select-button"[^>]*><\/button>\s*\$\{getThumbnailImageMarkup\(photo\.url, getPhotoFallbackLabel\(photo\), \{ sizes: '\(max-width: 860px\) calc\(\(100vw - 48px\) \/ 2\), 300px' \}\)\}\s*<\/article>/s);
    ...
    assert.match(likedRenderer, /<article class="personal-photo-card liked-photo-card"[^>]*>\s*\$\{getThumbnailImageMarkup\(photo\.url, getPhotoFallbackLabel\(photo\), \{ sizes: '\(max-width: 860px\) calc\(\(100vw - 48px\) \/ 2\), 300px' \}\)\}\s*<\/article>/s);
});

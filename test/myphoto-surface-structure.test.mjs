...
test('home starts the landing flow with the Ikkyee collage and feature stories', () => {
    ...
    assert.match(styles, /\.home-houses-reference::before\s*\{[^}]*z-index:\s*1;[^}]*background-image:\s*url\("images\/landing-compass-map\.jpg"\);[^}]*background-position:\s*center 88px;[^}]*background-repeat:\s*no-repeat;[^}]*opacity:\s*1;/s);
    ...
    assert.match(styles, /\.home-houses-reference__collage\s*\{[^}]*z-index:\s*3;[^}]*height:\s*560px;[^}]*margin-top:\s*78px;/s);

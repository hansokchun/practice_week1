...
test('home archive creation actions sit in their matching section headers', () => {
    ...
    assert.match(recentHeader, /id="recent-photo-title"[^>]*class="dashboard-section-title"[\s\S]*home-title-symbol[\s\S]*image[\s\S]*최근 사진/);
    assert.match(home, /id="liked-photo-title"[^>]*class="dashboard-section-title"[\s\S]*home-title-symbol[\s\S]*favorite[\s\S]*좋아요한 사진/);
    assert.match(albumHeader, /id="my-albums-title"[^>]*class="dashboard-section-title"[\s\S]*home-title-symbol[\s\S]*map[\s\S]*여행 앨범/);

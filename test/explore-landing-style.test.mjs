...
test('Explore map controls follow the landing surface and accent language', () => {
    ...
    assert.match(css, /\.explore-map\s*\{[^}]*outline:\s*none;[^}]*-webkit-tap-highlight-color:\s*transparent;[^}]*user-select:\s*none;/s);
    assert.match(css, /\.explore-map:focus,[\s\S]*\.explore-map \.gm-style \*:focus-visible\s*\{[^}]*outline:\s*none;/s);

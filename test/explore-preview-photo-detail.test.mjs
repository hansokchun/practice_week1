...
test('Explore pin preview sits in the right discovery panel position', () => {
    assert.match(css, /\.explore-pin-preview\s*\{[^}]*top:\s*16px;[^}]*right:\s*16px;[^}]*left:\s*auto;/s);
    assert.match(css, /\.explore-pin-preview\s*\{[^}]*width:\s*min\(390px,\s*calc\(100% - 56px\)\);/s);
    assert.match(css, /\.explore-pin-preview\s*\{[^}]*height:\s*clamp\(560px,\s*calc\(100svh - 32px\),\s*900px\);/s);
    assert.match(css, /\.explore-pin-preview\s*\{[^}]*max-height:\s*calc\(100svh - 32px\);/s);
    assert.match(css, /\.explore-discovery-panel\s*\{[^}]*top:\s*16px;[^}]*right:\s*16px;/s);
...
test('Explore photo preview does not render nearby photo thumbnails', () => {
    ...
    assert.doesNotMatch(body, /renderExplorePreviewNearby\(photo\)/);
    assert.doesNotMatch(source, /function renderExplorePreviewNearby\(/);
    assert.doesNotMatch(source, /data-pin-preview-nearby-photo/);
    assert.doesNotMatch(css, /\.pin-preview-nearby/);
});
...
test('Explore expanded own-photo preview edits description and visibility without changing location accuracy', () => {
    ...
    assert.match(saveBody, /updatePhotoInfo\(photo\.id, \{\s*description,\s*visibility\s*\}\)/s);
    assert.doesNotMatch(saveBody, /location_precision/);

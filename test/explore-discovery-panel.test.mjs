...
test('Explore discovery items use a consistent preview-sized frame without inline metadata', () => {
    ...
    assert.match(renderer, /<span class="explore-discovery-image">[\s\S]*<img src="\$\{escapeHtml\(photo\.url \|\| photo\.albumCoverUrl \|\| 'images\/main_bg2\.jpg'\)\}" alt="\$\{escapeHtml\(description \|\| label\)\}">/);
...
test('Explore discovery items keep descriptions only as accessibility labels', () => {
    ...
    assert.match(renderer, /<img src="\$\{escapeHtml\(photo\.url \|\| photo\.albumCoverUrl \|\| 'images\/main_bg2\.jpg'\)\}" alt="\$\{escapeHtml\(description \|\| label\)\}">/);
    assert.doesNotMatch(renderer, /explore-discovery-copy/);
    assert.doesNotMatch(renderer, /explore-discovery-time/);

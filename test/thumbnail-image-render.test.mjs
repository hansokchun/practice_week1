...
test('overview thumbnails show full photos inside rounded frames', () => {
    const css = stylesheet();

    assert.match(css, /\.recent-photo-grid img\s*\{[^}]*width:\s*100%;[^}]*height:\s*100%;[^}]*background:\s*var\(--surface-soft\);[^}]*object-fit:\s*contain;[^}]*object-position:\s*center;/s);
    assert.match(css, /\.personal-photo-card img\s*\{[^}]*width:\s*100%;[^}]*aspect-ratio:\s*1;[^}]*background:\s*var\(--surface-soft\);[^}]*object-fit:\s*contain;[^}]*object-position:\s*center;/s);
    assert.match(css, /\.explore-discovery-item img\s*\{[^}]*width:\s*100%;[^}]*height:\s*100%;[^}]*background:\s*var\(--bg\);[^}]*object-fit:\s*contain;[^}]*object-position:\s*center;/s);
});

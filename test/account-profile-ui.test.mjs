...
test('personal profile modal includes nickname, bio, and avatar file upload fields', () => {
    ...
    assert.match(html, /id="profile-nickname-input"/);
    assert.match(html, /id="profile-bio-input"/);
    assert.match(html, /id="profile-avatar-input"/);
    ...
    assert.doesNotMatch(html, /profile-website-input/);
    assert.doesNotMatch(html, /profile-avatar-url-input/);
});

...
test('mobile Explore uses a map-first canvas with a bottom-sheet preview', () => {
    ...
    assert.match(mobile, /\.explore-mobile-list-toggle\s*\{[^}]*display:\s*inline-flex;[^}]*left:\s*50%;[^}]*bottom:\s*calc\(18px \+ env\(safe-area-inset-bottom\)\);[^}]*width:\s*52px;[^}]*height:\s*52px;[^}]*border-radius:\s*999px;[^}]*transform:\s*translateX\(-50%\);/s);
...
test('mobile album and photo detail views avoid side-by-side desktop layouts', () => {
    ...
    assert.match(mobile, /\.photo-detail-like-panel\s*\{[^}]*justify-content:\s*flex-start;[^}]*gap:\s*4px;[^}]*padding:\s*5px 0;/s);

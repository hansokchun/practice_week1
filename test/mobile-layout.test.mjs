...
test('mobile Home keeps the private workspace visible without logged-in intro sections', () => {
    ...
    assert.match(mobile, /\.editorial-feature--korean\s+\.editorial-feature__side-image\s*\{[^}]*position:\s*absolute;[^}]*top:\s*28px;[^}]*right:\s*0;[^}]*width:\s*232px;/s);
    assert.match(mobile, /\.editorial-feature--korean\s+\.editorial-feature__headline\s*\{[^}]*width:\s*107%;[^}]*font-size:\s*212px;[^}]*white-space:\s*nowrap;/s);
    assert.match(mobile, /\.editorial-feature--korean\s+\.editorial-feature__statement\s*\{[^}]*width:\s*90%;[^}]*font-size:\s*156px;[^}]*white-space:\s*nowrap;/s);

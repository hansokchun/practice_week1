...
test('mobile Home keeps the private workspace and bottom reference visible while hiding top logged-in promos', () => {
    ...
    assert.match(mobile, /\.home-feature-story--explore\s+\.home-feature-story__media\s*\{[^}]*aspect-ratio:\s*auto;[^}]*border-radius:\s*0;/s);
    assert.match(mobile, /\.home-feature-story--explore\s+\.home-feature-story__media img\s*\{[^}]*padding:\s*0;/s);

...
| `--thumbnail-radius` | `8px` | Home and Explore photo thumbnail corners |
...
- **Discovery Cards**: Public photo cards in the Explore discovery panel render as image-only thumbnail entries. Each thumbnail uses the panel width in a consistent 4:3 frame, matching the default preview photo rhythm so the list scans evenly. Overview thumbnails fill their archive frame so the panel does not show thin image strips; full photo inspection remains in the preview and fullscreen surfaces. Thumbnail corners use the shared 8px thumbnail radius, while description or relative-time metadata appears only after opening the photo preview.
...
- **Image Fit**: Overview thumbnails fill the fixed archive frame to avoid narrow letterboxed strips; full-photo inspection surfaces use contained images when users need to see the complete original.

...
- **Discovery Cards**: Public photo cards in the Explore discovery panel render as image-only thumbnail entries. Each thumbnail uses the panel width in a consistent 4:3 frame, matching the default preview photo rhythm so the list scans evenly. Images fit inside the frame without cropping or zooming, using a restrained thumbnail render size so the list stays crisp without feeling enlarged. Thumbnail corners use the shared 8px thumbnail radius, while description or relative-time metadata appears only after opening the photo preview.
...
- **Image Fit**: Overview thumbnails fit the complete photo inside the fixed rounded square frame without crop zoom, while using moderate Supabase render variants for a slight clarity lift.
